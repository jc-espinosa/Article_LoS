...
# Read training database with last observations for MEWS

dat_resume_sat <- read.csv2(paste0(root_input,"/dataset_last_observations.csv"))

# Calculate MEWS

mews_score <- dat_resume_sat %>% mutate(sbp_mews = cut(last_pres_sisto,c(-Inf,70,80,100,199,Inf),labels = c(3,2,1,0,2),right = TRUE),
                                        hr_mews = cut(last_freq_card,c(-Inf,40,50,100,110,129,Inf),labels = c(2,1,0,1,2,3),right = TRUE),
                                        resp_mews = cut(last_freq_resp,c(-Inf,9,14,20,29,Inf),labels = c(2,0,1,2,3),right = TRUE),
                                        temp_mews = cut(last_temp,c(-Inf,35,38.4,Inf),labels = c(2,0,2),right = TRUE), # celsius
                                        avpu_mews = ifelse(last_est_neur %in% c('1','nm'),0,last_est_neur)
)
mews_score <- mews_score %>% select(sbp_mews,hr_mews,resp_mews,temp_mews,
                                    avpu_mews)
mews_score <- apply(mews_score,2,as.numeric)
mews_score[is.na(mews_score)] <- 0
mews_score <- apply(mews_score,1,sum,na.rm=T)

# Definite data-set

dat_resume_sat <- dat_resume_sat %>% 
  dplyr::select(`Episodio (único)`,los,Deterioro,Sexo,Edad)
dat_resume_sat$MEWS <- mews_score
...
# Read testing database with last observations for MEWS

dat_resume_sat_ts <- readRDS(paste0(root_input,"/dat_resume_sat_ts.rds"))

# Calculate MEWS

mews_score_ts <- dat_resume_sat_ts %>% mutate(sbp_mews = cut(last_pres_sisto,c(-Inf,70,80,100,199,Inf),labels = c(3,2,1,0,2),right = TRUE),
                                              hr_mews = cut(last_freq_card,c(-Inf,40,50,100,110,129,Inf),labels = c(2,1,0,1,2,3),right = TRUE),
                                              resp_mews = cut(last_freq_resp,c(-Inf,9,14,20,29,Inf),labels = c(2,0,1,2,3),right = TRUE),
                                              temp_mews = cut(last_temp,c(-Inf,35,38.4,Inf),labels = c(2,0,2),right = TRUE), # celsius
                                              avpu_mews = ifelse(last_est_neur %in% c('1','nm'),0,last_est_neur)
)
mews_score_ts <- mews_score_ts %>% select(sbp_mews,hr_mews,resp_mews,temp_mews,
                                          avpu_mews)
mews_score_ts <- apply(mews_score_ts,2,as.numeric)
mews_score_ts[is.na(mews_score_ts)] <- 0
mews_score_ts <- apply(mews_score_ts,1,sum,na.rm=T)
...
dat_resume_sat_ts$MEWS <- mews_score_ts
